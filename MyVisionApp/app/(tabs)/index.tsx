...
const stopDetection = (customMessage = '已停止') => {
  ...
  // --- 畫面渲染 1：模式選擇畫面 ---
  // 當 activeMode 為 null 時，只顯示兩個大按鈕讓使用者選擇
  if (!activeMode) {
    return (
      <View style={styles.selectionContainer}>
        <Text style={styles.titleText}>請選擇防護模式</Text>

        <TouchableOpacity
          style={styles.selectionBtn}
          onPress={() => setActiveMode('pedestrian')}
        >
          <Text style={styles.selectionBtnText}>🚶 行人模式</Text>
        </TouchableOpacity>

        <TouchableOpacity
          style={styles.selectionBtn}
          onPress={() => setActiveMode('motorcycle')}
        >
          <Text style={styles.selectionBtnText}>🏍️ 機車模式</Text>
        </TouchableOpacity>
      </View>
    );
  }
  ...
  // --- 畫面渲染 2：相機與辨識畫面 ---
  // 當使用者選定模式後，顯示實際的相機預覽與控制按鈕
  return (
    <View style={styles.container}>
      {/* 頂部資訊列：顯示目前所處模式 */}
      <View style={styles.topBar}>
        <Text style={styles.topBarText}>
          當前模式：{activeMode === 'pedestrian' ? '🚶 行人模式' : '🏍️ 機車模式'}
        </Text>
      </View>

      {/* 相機預覽畫面 */}
      <CameraView style={styles.camera} facing="back" ref={cameraRef}>
        {/* 利用 map 迴圈，將後端回傳的所有物件框繪製在相機畫面上 */}
        {boxes.map((box, i) => (
          <View
            key={i}
            style={[styles.box, {
              // 座標轉換邏輯：(後端座標 / 原始圖片尺寸) * 手機螢幕尺寸
              // 註: 因為下方有控制列，所以高度映射為螢幕高度的 75% (height * 0.75) 以符合比例
